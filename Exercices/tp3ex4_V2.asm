...
question:	db	"Tapez 0 pour des tableaux prédéfinis ou 1 pour remplir les tableaux : ",0
...
previous:	db	0
tabpredef1:	db	5,6,9,12,15,18,22,34,42,51
tabpredef2:	db	7,9,14,19,48
ind1:	db	0
ind2:	db	0
indfin:	db	0
...
boucle_demande_tab2:                
	...
	;**********************************************
	; on utilisera les tableaux prédéfinis dans .data
	; on les recopie donc dans tab1 et tab2
	;**********************************************
predefini:
	copie_tab1:
	movzx ecx,byte[i]
	mov al,byte[tabpredef1+ecx*BYTE]
	mov byte[tab1+ecx*BYTE],al
	inc byte[i]
	cmp byte[i],10
	jb copie_tab1
	...
	copie_tab2:
	movzx ecx,byte[i]
	mov al,byte[tabpredef2+ecx*BYTE]
	mov byte[tab2+ecx*BYTE],al
	inc byte[i]
	cmp byte[i],5
	jb copie_tab2
